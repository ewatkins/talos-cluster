...
| Image | `ghcr.io/starosdev/scrutiny` (`1.69.2-omnibus`, digest-pinned) | Web + API + embedded InfluxDB. **Fork**, not analogj — adds MegaRAID/HBA detection. Pinned to a versioned fork release rather than the floating `latest-omnibus` tag so Renovate raises real version bumps. Stay on the fork's `1.x-omnibus` line (not upstream's tagged 1.9.0): only the fork's `SmartSupport` type parses both bool (SATA) and object (NVMe) `smart_support`, so NVMe drives can register. |
