...
[`app/theme.css`](app/theme.css) is a VS Code Dark+ palette plus the document-header trim, and
it is applied to **everyone, on every page**, including the login page.

Alexandrie's own style injection cannot do that. `stylesInjection` is a per-user account
preference living in the `advanced` JSON column of `user_settings`, and `useStyleInjection()`
has exactly one caller — `pages/dashboard.vue` — which mounts the `<style>` elements and
removes them again in `onScopeDispose`. So it is post-auth, per-browser, and absent from every
public route. There is no instance-wide equivalent and no environment variable for one.

So the stylesheet is added to the markup on the way out, by an nginx sidecar:

```
gateway :8202 -> theme (nginx) -> 127.0.0.1:8200 (nuxt)
```

`sub_filter` rewrites `</head>` into `<link rel="stylesheet" href="/__theme.css"></head>`, and
the sidecar serves that path from the mounted file. Both files come from the
`alexandrie-theme` ConfigMap, generated by kustomize from `nginx.conf` and `theme.css`;
`disableNameSuffixHash` keeps the name stable, so the controller's reloader annotation is what
restarts the pod when either changes.

> **The HTTPRoute must point at 8202, not 8200.** 8200 is the Nuxt container and still
> answers — routing to it silently serves the app unthemed, which looks like the CSS is
> broken rather than bypassed. The `http` Service port was repointed for the same reason.

> **`proxy_set_header Accept-Encoding ""` is load-bearing.** `sub_filter` operates on the
> response body and cannot rewrite a compressed one; without it the injection silently does
> nothing. The gateway still compresses on the way to the browser.

Three things the CSS itself depends on:

- **Every declaration needs `!important`**, custom properties included. The app's own styles
  are entirely unlayered — there is no `@layer` anywhere in `app/styles/` — and the injected
  `<link>` needs to beat them.
...
> **A personal override still wins over this file.** Per-user CSS is wrapped in
> `@layer user-app`, and for `!important` declarations layer order is reversed — the unlayered
> injected sheet is the weakest of the two. Anything left in *Settings → Styles* will quietly
> override the default; clear it there.
