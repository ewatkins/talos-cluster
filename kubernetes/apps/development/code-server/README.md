...
There is deliberately **no stored `talosconfig`**. It is derived data (gitignored, generated by talhelper from `talsecret.sops.yaml`), so it is regenerated from the repo instead of held as a copy that can drift. `talhelper` reads `talconfig.yaml` in the current directory, so run it from the repo's `talos/` dir, then merge the result into the standard location `TALOSCONFIG` points at (`~/.talos/config`) — clone-location independent, so it works regardless of where you cloned:

```bash
cd ~/<clone>/talos
talhelper genconfig                                   # decrypts talsecret.sops.yaml with the mounted Age key
talosctl config merge ./clusterconfig/talosconfig     # -> ~/.talos/config, endpoints + nodes populated
talosctl version                                      # confirm it reaches the cluster
```

`kubectl` needs no kubeconfig at all — it falls back to the pod's ServiceAccount.
